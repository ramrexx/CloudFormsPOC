###################################
#
# CFME Automate Method: VMware_MarkVMAsTemplate
#
# Notes: This method marks a VMware VM as a template
#
###################################
begin
  # Method for logging
  def log(level, msg)
    @method = 'VMware_MarkVMAsTemplate'
  ...
  def call_vSphere(cookie, server, ems_ref)
    ...
    # Post the request
    resp, data = http.post(path, data, headers)
    log(:info, "MarkAsTemplate Response: #{resp.code}")
  end
  ...
  if vm.vendor.downcase == 'vmware'
    ...
    log(:info, "Marking VM: #{vm.name} as a template")
