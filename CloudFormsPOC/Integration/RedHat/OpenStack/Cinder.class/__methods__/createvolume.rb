# createVolume.rb
#
# Description: create a volume in OpenStack
#

#gem 'fog', '>=1.22.0'
require 'fog'

def get_tenant
  tenant_ems_id = $evm.root['dialog_cloud_tenant']
  $evm.log(:info, "Found EMS ID of tenant from dialog: #{tenant_ems_id}")
  return tenant_ems_id if tenant_ems_id.nil?

  tenant = $evm.vmdb(:cloud_tenant).find_by_id(tenant_ems_id)
  $evm.log(:info, "Found EMS Object for Tenant from vmdb: #{tenant.inspect}")
  return tenant
end

$evm.root.attributes.sort.each { |k, v| $evm.log(:info, "Root:<$evm.root> Attribute - #{k}: #{v}")}

service_template_provision_task = $evm.root['service_template_provision_task']
service = service_template_provision_task.destination
$evm.log(:info, "Detected Service:<#{service.name}> Id:<#{service.id}> Tasks:<#{service_template_provision_task.miq_request_tasks.count}>")
$evm.log(:info, "DEBUG: #{service_template_provision_task.inspect}")

mid = $evm.root['dialog_mid']
raise "Management System ID is nil" if mid.blank?
openstack = nil

unless mid.blank?
  openstack = $evm.vmdb(:ems_openstack).find_by_id(mid)
else
  openstack = $evm.vmdb(:ems_openstack).all.first
end

raise "OpenStack Management system with id '#{mid}' not found" if openstack.nil?
$evm.log(:info, "EMS_Openstack: #{openstack.inspect}\n#{openstack.methods.sort.inspect}")

name = $evm.root['dialog_instance_name'] + 'vol1'
description = "Volume #{name} created by CloudForms at #{Time.now}"
size = $evm.root['dialog_cinder_size'].to_i

tenant = get_tenant
$evm.log(:info, "Using tenant: #{tenant.name}")

conn = Fog::Volume.new({
                         :provider => 'OpenStack',
                         :openstack_api_key => openstack.authentication_password,
                         :openstack_username => openstack.authentication_userid,
                         :openstack_auth_url => "http://#{openstack[:hostname]}:#{openstack[:port]}/v2.0/tokens",
                         :openstack_tenant => tenant.name
})

$evm.log(:info, "Successfully connected to Storage Service at #{openstack.name}")
$evm.log(:info, "Creating volume #{name}/#{description} of size #{size}GB")

imageref = $evm.root['dialog_glance_imageref']
volume = nil
if imageref.blank?
  volume = conn.create_volume(name, description, size, {:bootable => true}).body['volume']
else
  $evm.log(:info, "Cloning image from image #{imageref}")
  volume = conn.create_volume(name, description, size, { :bootable => true, :imageRef => imageref }).body["volume"]
end

$evm.log(:info, "Create Volume Response: #{volume.inspect}")

$evm.set_state_var(:volume, volume)

# service.name = "VOLUME: #{name} #{size}GB"
# service.description = "Cinder Volume with ID #{volume['id']} of size #{size}GB"
# service.custom_set("VOLUME_ID", "#{volume['id']}")
# service.custom_set("MID", mid)
# service.custom_set("IMAGEREF", "#{imageref}") unless imageref.blank?
# service_template_provision_task.set_option(:volume_id, volume['id'])
# service_template_provision_task.set_option(:mid, mid)
# service_template_provision_task.set_option(:tenant_name, tenant.name)
# service.tag_assign("cloud_tenants/#{tenant.name}")

$evm.log(:info, "Created #{service.name}/#{service.description}")
$evm.log(:info, "DETAILS: #{service_template_provision_task.inspect}")
