...
vm_guid = $evm.get_state_var(:vm_guid)
...
$evm.log(:info, "User: #{user.name} Group: #{group.description}")

vm.owner = user
vm.group = group

$evm.log(:info, "VM: #{vm.name} Owner: #{vm.owner.name} Group: #{vm.owner.current_group.description}")
