###################################
#
# CFME Automate Method: VMware_MountToolsInstaller
#
# Notes: This method mounts the VMware Tools CD installer as a CD-ROM for the guest operating system
#
...
begin
  # Method for logging
  def log(level, msg)
    @method = 'VMware_MountToolsInstaller'
  ...
  def call_vSphere(cookie, server, ems_ref)
    ...
    # Post the request
    resp, data = http.post(path, data, headers)
    log(:info, "MountToolsInstaller Response: #{resp.code}")
  end
  ...
  if vm.vendor.downcase == 'vmware'
    ...
    log(:info, "Mounting VMware Tools on VM: #{vm.name}")
