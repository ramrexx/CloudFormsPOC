###################################
#
# CFME Automate Method: UnMount_ISO
#
# Notes: This method unmounts an ISO for a VMware VM.
# https://www.vmware.com/support/developer/vc-sdk/visdk400pubs/ReferenceGuide/vim.vm.ConfigSpec.html
# Inputs:
#
###################################
begin
  ...
  def unmount_iso(provider, vm)
    ...
    backing_filename = cdrom.backing.fileName rescue nil
    unless backing_filename.nil?
      log(:info, "UnMounting ISO: #{backing_filename} on VM: #{vm.name}")

      machine_conf_spec = RbVmomi::VIM::VirtualMachineConfigSpec(
        deviceChange: [{
                         operation: :edit,
                         device: RbVmomi::VIM::VirtualCdrom(
                           backing: RbVmomi::VIM::VirtualCdromRemotePassthroughBackingInfo(:deviceName => '', :exclusive => false ),
                           key: cdrom.key,
                           controllerKey: cdrom.controllerKey,
                           connectable: RbVmomi::VIM::VirtualDeviceConnectInfo(:startConnected => false, :connected => false, :allowGuestControl => true)
      )}])
      vsphere_vm.ReconfigVM_Task(spec: machine_conf_spec).wait_for_completion
    else
      log(:warn, "No ISO file found")
      return
    end
  end
  ...
  log(:info, "VM: #{vm.name} vendor: #{vm.vendor} provider: #{provider.name} datacenter: #{vm.v_owning_datacenter}")

  if vm.vendor.downcase == 'vmware'
    unmount_iso(provider, vm)
    vm.custom_set(:CDROM, nil)
  end
