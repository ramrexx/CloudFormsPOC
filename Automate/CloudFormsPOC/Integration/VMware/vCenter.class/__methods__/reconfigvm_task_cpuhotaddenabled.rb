# ReconfigVM_Task_cpuHotAddEnabled.rb
#
# Description: This method changes a VMware VM's cpuHotAddEnabled switch to true or false
#
require 'savon'
...
$evm.root['dialog_cpuHotAddEnabled'].nil? ? (cpuHotAddEnabled = true) : (cpuHotAddEnabled = $evm.root['dialog_cpuHotAddEnabled'])
...
reconfig_vm_task_result = client.call(:reconfig_vm_task) do
  message( '_this' => vm.ems_ref, :attributes! => { 'type' => 'VirtualMachine' },
    'spec' => { 'cpuHotAddEnabled' => true }, :attributes! => { 'type' => 'VirtualMachineConfigSpec' }  ).to_hash
end
